...
os.system("") #https://blog.csdn.net/ytzlln/article/details/81945246 win10字体颜色显示不了的问题
...
def log(message, level=LOG_LEVEL_NORMAL, noident=False, bold=False):
    ...
    if level >= LOG_LEVEL:
        is_term = True
        #get_env('TERM') is not None or get_env('USER') == '91act'
...
def get_res_path():
    return get_env('SCRIPT_RES_PATH', get_script('../Assets'))

def get_unity_project():
    return get_env('SCRIPT_RES_PATH', get_script('../'))
...
def zip_files(path, pack_size=4 * 0x100000, filenametag=''):
    ...
    for uncompressed_files in zip_packs:
        if len(uncompressed_files) > 0:
            ...
            if filenametag != '' and filenametag != None:
                execute('zip', '-q', 'update_{0}_{1}_{2}_{3}'.format(timestamp, filenametag, zip_index, file_hash(uncompressed_files)), '-@', '<', list_file)
            else:
                execute('zip', '-q', 'update_{0}_{1}_{2}'.format(timestamp, zip_index, file_hash(uncompressed_files)), '-@', '<', list_file)
            zip_index += 1
...
def deploy(src_path, dst_path, files, remove_diff=True):
    ...
    if "91ACT_CONTROL" in dst_path:
        for item in files:
            info("deploy.foreach %s"%(item))
...
initialize(dir_name(__file__ + "/scripts"))