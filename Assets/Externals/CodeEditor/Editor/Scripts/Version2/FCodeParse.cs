...
namespace CodeEditor2
{
    public static class FCodeParse
    {
        ...
        static void ParseInstances(Dictionary<int, List<Token>> line2Tokens, int startLine, int endLine, ref FCodeLogic logic)
        {
            FCodeWindow w = EditorWindow.GetWindow<FCodeWindow>();
            Dictionary<string, string> fcodeSignature = w.fcodeSignature;
            var isInstancesStarted = false;
            List<FCodeLogic.InstanceConfig.ConfigVal> currentInstancesDict = null;
            for (int currentLine = startLine + 1; currentLine < endLine; currentLine ++)
            {
                if (line2Tokens.TryGetValue(currentLine, out List<Token> lineContent))
                {
                    // ** 从签名开始处 
                    if (!isInstancesStarted)
                    {
                        isInstancesStarted = ValidSignature(lineContent, fcodeSignature, "InstancesStart");
                        continue;
                    }
                    else if(ValidSignature(lineContent, fcodeSignature, "InstancesStart"))
                    {
                        Error("重复的FCodeSignature:Instances:Start签名？？");
                    }
                    // ** 把签名中的instances名称和其值都加到logic中，暂时用=符号来区分是instance名称还是值
                    var index = lineContent.FindIndex(v => v.Original().Equals("="));
                    if( index == lineContent.Count - 1)
                    {
                        var nameToken = lineContent.Find(v => v is NameToken);
                        if (nameToken == null)
                        {
                            nameToken = lineContent.Find(v => v is StringToken);
                        }
                        var name = nameToken.Original();
                        if (logic.instances.Find(v => v.instanceName.Equals(name)) == null)
                        {
                            var newInstance = new FCodeLogic.InstanceConfig()
                            {
                                instanceName = name,
                            };
                            logic.instances.Add(newInstance);
                            currentInstancesDict = newInstance.values;
                        }
                        else
                        {
                            Debug.LogWarning($"存在同名的instances？？{name}");
                        }
                    }
                    else if (index > 0 && currentInstancesDict != null)
                    {
                        var valueToken = lineContent[index + 1];
                        var nameToken = lineContent.Find(v => v is NameToken);
                        if (nameToken == null)
                        {
                            nameToken = lineContent.Find(v => v is StringToken);
                        ...
                        var name = nameToken.Original();
                        if (valueToken is NumberToken nToken)
                        {
                            currentInstancesDict.Add(new FCodeLogic.InstanceConfig.ConfigVal()
                            {
                                key = name,
                                type = FCodeLogic.InstanceConfigType.Number,
                                value = nToken.Original(),
                            });
                        }
                        else if (valueToken is StringToken)
                        {
                            currentInstancesDict.Add(new FCodeLogic.InstanceConfig.ConfigVal()
                            {
                                key = name,
                                type = FCodeLogic.InstanceConfigType.String,
                                value = valueToken.Original(),
                            });
                        }
                        else if (valueToken is LiteralToken lToken && lToken.tokenType == '-')
                        {
                            var nToken1 = lineContent[index + 2] as NumberToken;
                            currentInstancesDict.Add(new FCodeLogic.InstanceConfig.ConfigVal()
                            {
                                key = name,
                                type = FCodeLogic.InstanceConfigType.Number,
                                value = $"-{nToken1.Original()}",
                            });
                        }
                        else
                        {
                    ...
                    // ** 到签名结束
                    if (ValidSignature(lineContent, fcodeSignature, "InstancesEnd"))
                    {
                ...
                foreach (var fcodeFunction in functions)
                {
                    ...
                    foreach (var entry in fcodeFunction.line2Tokens)
                    {
                        ...
                        if (tokens.Count > 2)
                        {
                            ...
                            {
                                parametersTypeRef.Add($"{fcodeFunction.shortName}_{varName}", first.Replace("Check", ""));
                            }
                        }
                    }
        ...
        public static FCodeLogic Tokenize(string fileName, string content)
        {
            ...
            if (result.TryGetValue("line2Tokens", out var ol))
            {
                ...
                // ** 找到有 instances 关键字 且 token 数量为2的行
                enties = line2Tokens.Where(item => item.Value.Select(v => v.Original() == "instances").Any()).ToArray();
                entry = enties.FirstOrDefault(item => item.Value.Count == 2);
                if (entry.Value != null)
                {
                    startLine = entry.Key;
                    ParseInstances(line2Tokens, startLine, (int)endLine, ref logic);
                }
            }
