...
namespace CodeEditor2
{
    ...
    public class FCodeTreeViewItem : CodeEditorTreeViewItem
    {
            ...
            switch (m_fcodeNodeType)
            {
                ...
                case FCodeNodeType.Function:
                    ...
                    for(int parameterIndex = 0; parameterIndex < fcodeFunction.functionParameters.Count; ++parameterIndex)
                    {
                        ...
                        FCodeWindow.instance.fcodeLib.parametersTypeRef.TryGetValue($"{fcodeFunction.shortName}_{paramName}", out string parameterTypeRef);
                        ...
                        {
                            isConfig = GUILayout.Toggle(isConfig, "是否为参数列表字段");
                        }
                        ...
                        if(isConfig)
                        {
                            ...
                            int i = -1;
                            var templateInstance = FCodeWindow.instance.fcodeTreeView.fcodeLogic.instances.Find(v => v.instanceName.Equals(FCodeLogic.templateInstance));
                            if (parameterConfig.fields != null)
                            {
                                i = templateInstance.values.FindIndex(p => p.key.Equals(parameterConfig.fields[0].Original()));
                            }
                            var array = templateInstance.values.Select(v => v.key).ToArray();
                            var ni = EditorGUILayout.Popup(i, array);
                            if (ni > -1)
                            {
                                if (ni != i)
                                {
                                    parameterConfig.fields = parameterConfig.fields ?? new List<Token>() { new NameToken("?") };
                                    parameterConfig.fields[0].SetOriginal(templateInstance.values.ToArray()[ni].key);
                                }
                            }
                            else
                            {
        ...
        public void ConfigNameRefresh(string oriConfigName,string newConfigName, bool delete = false)
        {
            Traverse((item) => {
                if (item is FCodeTreeViewItem treeViewItem && treeViewItem.data is ParsedTokens pt)
                {
                    if(pt.parameterConfigs != null)
                    {
                        foreach(var parameterConfig in pt.parameterConfigs)
                        {
                            if(parameterConfig.isConfig)
                            {
                                var token = parameterConfig.fields[0];
                                var original = token.Original();
                                if(original.Equals(oriConfigName))
                                {
                                    parameterConfig.isConfig = !delete;
                                    token.SetOriginal(newConfigName);
                                }
                            }
                        }
                        treeViewItem.DisplayNameRefresh();
                    }
                }
            });
        }
