...
partial class Node
{
    ...
    {
        get { return new Rect(nodePosition.x, nodePosition.y, size.x, size.y); }
        set
        {
            nodePosition = new Vector2((int)value.x, (int)value.y);
    ...
    {
        get { return Preferences.showIcons && icon != null; }
    }
    ...
    {
        var newGraph = EditorTools.CreateAsset<T>(true);
        if (newGraph != null)
        {
    ...
    void ShowHeader()
    {
        ...
        var colorizeHeader = Preferences.nodeHeaderStyle == Preferences.NodeHeaderStyle.ColorizeHeader;
        ...
        if (showIcon && (iconAlignment == Alignment2x2.Default || iconAlignment == Alignment2x2.Bottom))
    ...
    void ShowPossibleWarningIcon()
    {
        ...
        if (assignable != null && assignable.task != null)
        {
            ...
            if (warning != null)
            {
                ...
                GUI.Box(errorRect, EditorTools.warningIcon, GUIStyle.none);
            }
        }
    }
    ...
    void HandleEvents(Event e)
    {
        ...
        if (e.type == EventType.MouseDown && Graph.allowClick && e.button != 2)
        {
            ...
            if (e.button == 0 && e.clickCount == 2)
            {
                ...
                {
                    EditorTools.OpenScriptOfType((this as ITaskAssignable).task.GetType());
                }
                else
                {
                    EditorTools.OpenScriptOfType(this.GetType());
                }
                e.Use();
            }
    ...
    void ShowNodeContents()
    {
        ...
        if (this is ITaskAssignable)
        {
            ...
            {
                GUILayout.Label(Preferences.showTaskSummary ? task.summaryInfo : string.Format("<b>{0}</b>", task.name));
            }
        }
                ...
                if (this is ITaskAssignable)
                {
                    ...
                    if (Task.copiedTask != null)
                    {
                        menu.AddItem(new GUIContent("Paste Assigned Task"), false, () =>
                       {
                           ...
                           {
                               // ParadoxNotion.Services.Logger.LogWarning("Can't paste Task here. Incombatible Types", "Editor", this);
                           }
                       });
    ...
    void HandleNodePosition(Event e)
    {
        ...
        if (Graph.allowClick && e.button != 2)
        {
            ...
            if (nodeIsPressed)
            {
                ...
                var hierarchicalMove = Preferences.hierarchicalMove != e.shift;

                //snap to grid
                if (!hierarchicalMove && Preferences.doSnap && Graph.multiSelection.Count == 0)
                {
    ...
    void DrawNodeComments()
    {
        ...
        if (!Preferences.showComments || string.IsNullOrEmpty(nodeComment))
        {
            return;
        }
    ...
    void DrawNodeTag()
    {
        ...
        if (!string.IsNullOrEmpty(tag))
        {
            ...
            tagRect.width = EditorTools.tagIcon.width;
            tagRect.height = EditorTools.tagIcon.height;
            tagRect.y += tagRect.height + 5;
            tagRect.x = nodeRect.x - 22;
            GUI.DrawTexture(tagRect, EditorTools.tagIcon);
        }
    }
    ...
    public void ShowNodeInspectorGUI()
    {
        ...
        UndoTools.CheckUndo(graph, "Node Inspector");

        if (Preferences.showNodeInfo)
        {
        ...
        if (!showIcon && allowAsPrime)
        {
            ...
            EditorTools.TextFieldComment(customName, "Name...");
        }

        tag = EditorGUILayout.TextField(tag);
        EditorTools.TextFieldComment(tag, "Tag...");
        ...
        GUI.color = Color.white;
        EditorTools.TextFieldComment(nodeComment, "Comments...");
    ...
    void TaskAssignableGUI()
    {
        ...
        if (this is ITaskAssignable)
        {
            ...
            if (taskType != null)
            {
                EditorTools.TaskField(assignable.task, graph, taskType, (t) =>
                {
                    _icon = null;
                    assignable.task = t;
                });
    ...
    {
        EditorTools.ShowAutoEditorGUI(this);
    }
    ...
    virtual protected void DrawNodeConnections(Rect drawCanvas, bool fullDrawPass, Vector2 canvasMousePos, float zoomFactor)
    {
        ...
        if (clickedPort != null && e.type == EventType.MouseUp && e.button == 0)
        {
            ...
            else
            {
                ...
                if (dragDropMisses == graph.allNodes.Count && clickedPort != null)
                {
                    ...
                    var menu = EditorTools.GetTypeSelectionMenu(graph.baseNodeType, Selected);
                    if (zoomFactor == 1 && Preferences.useBrowser)
                    {
        ...
        for (var i = 0; i < outConnections.Count; i++)
        {
            ...
            if (connection != null)
            {
                ...
                var boundRect = RectTools.GetBoundRect(sourcePortRect, targetPortRect);
