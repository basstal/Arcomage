...
abstract public partial class Connection
{
    ...
    public static Connection Create(Node source, Node target, BevNode sourceNode, BevConnection bevConnection)
    {
        var sourceIndex = sourceNode.OutConnectionsUid.IndexOf(bevConnection.Uid);
        var connection = Create(source, target, sourceIndex, true);
        connection._UID = bevConnection.Uid;
        return connection;
    }
    public static Connection Create(Node source, Node target, int sourceIndex, bool onSerialization = false)
    {
        ...
        if (source.graph != null && !onSerialization)
        {
    ...
    public Status Execute(Component agent)
    {
        ...
        status = targetNode.Execute(agent);
        return status;
    }
    ...
    {
        return JsonUtility.FromJson<Connection>(JsonUtility.ToJson(this));
    }
    public BevConnection ToBevConnection()
    {
        var bevConnection = new BevConnection();
        bevConnection.Uid = UID;
        bevConnection.SourceNodeUid = sourceNode.UID;
        bevConnection.TargetNodeUid = targetNode.UID;
        bevConnection.IsActive = isActive;
