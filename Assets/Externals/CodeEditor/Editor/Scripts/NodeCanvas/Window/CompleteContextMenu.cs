...
public class CompleteContextMenu : PopupWindowContent
{
    ...
    class Node
    {
        public EditorTools.MenuItemInfo item = null;
    ...
    private static EditorTools.MenuItemInfo[] items;
    ...
    public static void SetMenu(GenericMenu newMenu)
    {
        ...
        generationThread = ThreaderTools.StartAction(GenerateTree, () => { generationThread = null; current.editorWindow.Repaint(); });
    }
    ...
    private static void LoadPrefs()
    {
        if (data == null)
        {
            ...
            if (!string.IsNullOrEmpty(json)) { data = JsonUtility.FromJson<SerializationData>(json); }
    ...
    private static void SavePrefs()
    {
        ...
        EditorPrefs.SetString(PREFERENCES_KEY, JsonUtility.ToJson(data));
    }
    ...
    {
        loadProgress = 0;
        items = EditorTools.GetMenuItems(boundMenu);
    ...
    void DoTree(Rect treeRect, Event e)
    {
        ...
        foreach (var childPair in currentNode.children)
        {
            ...
            var icon = leafItem != null ? leafItem.content.image : EditorTools.folderIcon;
            ...
            if (icon != null)
            {
                ...
                if (typeInfo != null && icon.name == UserTypePreferences.DEFAULT_TYPE_ICON_NAME)
                {
                    GUI.color = UserTypePreferences.GetTypeColor(typeInfo);
                }
            }
            ...
            if (currentKeyType != null)
            {
                ...
                if (GUILayout.Button(EditorTools.favoriteIcon, GUIStyle.none, GUILayout.Width(16), GUILayout.Height(16)))
                {
                    node.ToggleFavorite();
                }
                GUI.color = Color.white;
    ...
    void ExecuteItemFunc(EditorTools.MenuItemInfo item)
    {
        if (item.func != null)
        {
            item.func();
