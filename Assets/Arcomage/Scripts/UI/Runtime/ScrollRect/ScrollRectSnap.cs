...
namespace NOAH.UI
{
    ...
    public class ScrollRectSnap : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
    {
        ...
        [Serializable]
        public class BeginSnapEvent : UnityEvent<Vector2> { }
        ...
        [SerializeField]
        protected DG.Tweening.Ease m_EaseType = DG.Tweening.Ease.Linear;
        public DG.Tweening.Ease easeType { get { return m_EaseType; } set { m_EaseType = value; } }
        [SerializeField]
        ...
        private DG.Tweening.Tween m_SnapTween = null;

        public float m_distanceLimitForElasticMovement = 0;
        ...
        public void SnapToStart(float? _smoothTime = null, DG.Tweening.Ease? _easeType = null)
        {
        ...
        public void SnapToEnd(float? _smoothTime = null, DG.Tweening.Ease? _easeType = null)
        {
            SnapToEdge(false, _smoothTime, _easeType);
        }
        
        protected void SnapToEdge(bool start, float? _smoothTime, DG.Tweening.Ease? _easeType)
        {
        ...
        {
            if (autoSnap && !m_Dragging && m_SnapTween == null && gameObject.activeInHierarchy)
            {
                var scrollAxis = scrollRect.vertical ? 1 : 0;
                ...
                // skip elasticity process; skip inertial process until speed drops down to the threshold
        ...
        public void SnapRelative(Vector2 offset, float? _smoothTime = null, DG.Tweening.Ease? _easeType = null)
        {
            if (gameObject.activeInHierarchy)
            {
                ...
                m_OnBeginSnap.Invoke(offset);

                if (type == DG.Tweening.Ease.INTERNAL_Custom)
                {
                    m_SnapTween = scrollRect.content.DOAnchorPos(scrollRect.content.anchoredPosition + offset, time).SetEase(easeCurve).SetUpdate(true).OnComplete(() =>
                    {
                        if (IsActive())
                        {
                            StartCoroutine(DelayStopSnap());
                ...
                {
                    m_SnapTween = scrollRect.content.DOAnchorPos(scrollRect.content.anchoredPosition + offset, time).SetEase(type).SetUpdate(true).OnComplete(() =>
                    {
                        if (IsActive())
                        {
                            StartCoroutine(DelayStopSnap());
        ...
        // delay StopSnap(set m_SnapTween to null) to avoid SnapRelative invoked next frame by OnScrolling->TrySnap->SnapRelative
        protected IEnumerator DelayStopSnap()
        {
            yield return null;
            yield return null; // two frames required.
        ...
        public void SnapTo(uint index, float? _smoothTime = null, DG.Tweening.Ease? _easeType = null)
        {
            SnapRelative(GetSnapOffsetByIndex(index), _smoothTime, _easeType);
        }

        public void SnapTo(RectTransform target,Vector2 normalizedOffset, float? _smoothTime = null, DG.Tweening.Ease? _easeType = null)
        {
            //var targetSnapPos = GetRelativePosition(scrollRect.viewport, target, 1, targetSnapOffset);
            Vector2 offset = Vector2.zero;
            Rect viewPortRect = scrollRect.viewport.rect;
            Rect targetRect = target.rect;

            Vector3 relativePos = scrollRect.viewport.InverseTransformPoint(target.TransformPoint(targetRect.min.x,targetRect.max.y,0));

            float height = viewPortRect.height - targetRect.height;
            float width = viewPortRect.width - targetRect.width;
            offset.y = normalizedOffset.y * height;
            offset.x = normalizedOffset.x * width;
            //clame to 0~+inf
            offset.y = offset.y < 0 ? 0 : offset.y;
            offset.x = offset.x < 0 ? 0 : offset.x;
            offset.y = viewPortRect.max.y - relativePos.y - offset.y;
            offset.x = viewPortRect.min.x - relativePos.x - offset.x;
            
            SnapRelative(offset, _smoothTime, _easeType);
        }
        ...
        void Update()
        {
            if (m_distanceLimitForElasticMovement > 0)
            {
                float maxX = (targetParent.rect.width - scrollRect.viewport.rect.width) * 0.5f + m_distanceLimitForElasticMovement;
