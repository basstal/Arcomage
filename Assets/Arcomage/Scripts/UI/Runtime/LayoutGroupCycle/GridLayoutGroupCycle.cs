...
namespace NOAH.UI
{
    [RequireComponent(typeof(UISizeFitter))]
    public class GridLayoutGroupCycle : GridLayoutGroup, ILayoutGroupCycle
    {
        ...
        public delegate void RectTransformDimensionsChange(float width, float height);
        public RectTransformDimensionsChange onRectTransformDimensionsChange { get; set; }
        ...
        protected override void Awake()
        {
            ...
            if (scrollRect != null)
            {
                ...
                {
                    // LogTool.LogError("Layout", "ScrollRect should have only one scroll dimension enabled while used with GridLayoutGroupCycle.");
            ...
            {
                // LogTool.LogError("Layout", "GridLayoutGroupCycle should be used with ScrollRect.");
            }
        ...
        protected virtual void OnScrolling(Vector2 normalizedPosition)
        {
        ...
        {
            var contentSizeFitter = GetComponent<UISizeFitter>();
            if (contentSizeFitter != null)
            {
                if (startAxis == Axis.Horizontal)
                {
                    contentSizeFitter.m_fitHorizontal = true;
                }
                else
                {
                    contentSizeFitter.m_fitVertival = true;
                }
            }
        ...
        protected void UpdateChildrenPositions(bool cycling = false)
        {
            if (scrollRect != null)
            {
                ...
                {
                    int cornerX = (int)startCorner % 2;
                    m_NormalizedPosition = cornerX == 0 ? scrollRect.normalizedPosition : Vector2.one - scrollRect.normalizedPosition;
                }
                else if (scrollRect.vertical)
                {
                    int cornerY = (int)startCorner / 2;
                    m_NormalizedPosition = cornerY == 0 ? Vector2.one - scrollRect.normalizedPosition : scrollRect.normalizedPosition;
                }
            }
        ...
        void LateUpdate()
        {
            ...
            if (m_PendingPopulateList.Count > 0)
            {
                m_PendingPopulateList.ForEach(go =>
                {
                    ...
                    // LogTool.LogInfo("Layout", $"GridLayoutGroupCycle.PopulateChild({index})");
                });
        ...
        override protected void OnRectTransformDimensionsChange()
        {
            ...
            onRectTransformDimensionsChange?.Invoke(rectTransform.rect.width, rectTransform.rect.height);
        }
    }
}
